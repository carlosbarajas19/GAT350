...
namespace PostProcess
{
	...
	void BoxBlur(const ColorBuffer& colorBuffer)
	{
		...
		for (size_t i = 0; i < colorBuffer.width * colorBuffer.height; i++)
		{
			int x = i % source.width;
			int y = i / source.width;

			if (x < 1 || x + 1 >= source.width || y < 1 || y > source.height) continue;
	...
	void GausianBlur(const ColorBuffer& colorBuffer)
	{
		ColorBuffer source = colorBuffer;
		int16_t k[3][3] =
		{
